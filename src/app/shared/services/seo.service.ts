...
export class SeoService {
   ...
   */
  setProductPageSEO(product: any, productSlug: string | null, baseUrl: string = 'https://FashionMaster4u.com'): void {
  ...
  private generateProductTitle(product: any): string {
    ...
    // Example: "Nike Air Max 270 Men's Running Shoes - ₹8,999 | FashionMaster4u"
    return `${brand}${product.name}${category} - ${price} | FashionMaster4u`;
  }
  ...
  private generateProductDescription(product: any): string {
    ...
    let rawDescription: string = product.meta_description
      ...
      || product.description
      || `Shop ${brand}${product.name}${category} online at FashionMaster4u. Premium quality, great prices, fast delivery.${discount}`;
  ...
  private sanitizeProductDescription(text: string): string {
    ...
    if (!sanitized) {
      sanitized = 'Premium quality, great prices, fast delivery from FashionMaster4u.';
    }
  ...
  private generateProductKeywords(product: any): string {
    ...
    keywords.push('buy online', 'FashionMaster4u', 'fashion', 'clothing');
